...
class Scene(QGraphicsScene):
    ...
    # ca permet de signaler le produit sous le curseur et sa position
    def __init__(self):
        super().__init__()
        ...
        self.point_depart = (29, 3)  # Point de départ mis à jour
    ...
    def charger_plan(self, chemin_image=None):
        # Charge le plan du magasin depuis un fichier image
...
class Rendu(QGraphicsView):
    ...
    # Permet de charger le plan du magasin
    def resizeEvent(self, event):
        super().resizeEvent(event)
        if self.scene() and self.scene().sceneRect().width() > 0:
            self.fitInView(self.scene().sceneRect(), Qt.AspectRatioMode.KeepAspectRatio)
    
    # Permet de centrer la vue sur un point donné
    def centrer_sur_point(self, point):
        if point: self.centerOn(point)
    
    # Permet de zoomer/dézoomer avec la molette de la souris
...
class MaxiMarketMainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # Création du splitter pour séparer le rendu et le panneau latéral
        splitter = QSplitter(Qt.Orientation.Horizontal)
        
        # Rendu du plan (le panneau gauche)
        self.rendu = Rendu()
        splitter.addWidget(self.rendu)
        
        # Panneau latéral ( le truc a droite)
        side_panel = QWidget()
    ...
    def ajouter_item_liste(self, produit): 
        self.liste_widget.addItem(produit)

    # supprime un produit de la liste de courses
    def vider_liste(self): 
        self.liste_widget.clear()
    
    # maj la liste de courses avec les produits
    ...
    # Vide la table des infos du chemin
    def vider_table_chemin(self):
        self.table_chemin.setRowCount(0)
    
    # Permet de récupérer le rendu du plan
    def get_rendu(self): 
        return self.rendu
    ...
    #  Permet de demander un fichier pour sauvegarder
